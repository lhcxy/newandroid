...
public class ImageChoosePop extends PopupWindow {

    private Context context;

    public ImageChoosePop(Context context) {
        super(context);
    ...
    private void initalize() {
        ...
        View view = inflater.inflate(R.layout.pop_image_choose_menus, null);
        view.findViewById(R.id.pop_take_photo_tv).setOnClickListener(v -> {
            dismiss();
            if (callback != null) {
                callback.onChose(0);
            }
        });
        view.findViewById(R.id.pop_take_album_tv).setOnClickListener(v -> {
            dismiss();
            if (callback != null) {
                callback.onChose(1);
            }
        });
        view.findViewById(R.id.pop_photo_cancel_tv).setOnClickListener(v -> dismiss());
        setContentView(view);
        initWindow();
    }
